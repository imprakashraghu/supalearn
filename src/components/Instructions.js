...
function Instructions() {
    ...
    const playSound = (src, symbol) => {
      ...
      // If the same item is clicked, stop and do not play again
      if (selectedElement === symbol) {
        setAudio(null);
        setSelectedElement(null);
        return;
      }
      const newAudio = new window.Audio(src);
      setAudio(newAudio);
      newAudio.play();
    };

    const playH = () => playSound(h, 'H');
    const playO = () => playSound(o, 'O');
    const playNa = () => playSound(na, 'Na');
    const playCl = () => playSound(cl, 'Cl');
    const playDefault = () => playSound(null, null);
  ...
  return (
    ...
    >
      <div className="w-full h-screen bg-orange-100 flex flex-col items-center justify-center">
        <div className="flex flex-row items-center mb-4">
          <h1
            className="bg-orange-100 font-bold tracking-tighter text-center text-lg px-4"
            style={{ fontSize: "24px" }}
          >
            Know Before You Play
          </h1>
          <button className="nes-btn is-warning" onClick={() => route('/lab')}>Continue</button>
        </div>
        <div className="max-w-[95%] mx-auto flex flex-wrap items-center justify-center gap-3 p-2">
          {elements.map((element, index) => ( 
              ...
              }} className={`text-xl ${selectedElement===element.symbol?'animate-bounce nes-btn is-error':''} nes-btn rounded-md border-4 border-black flex items-center text-black bg-white justify-center`}>
                {selectedElement===element.symbol?(`${element.name} (${element.number})`):element.symbol} 
              </div>
          ))}
        </div>
        <br/>
