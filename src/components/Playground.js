...
import ch from '../sounds/choose.mp3'
...
function Playground() {
  ...
  const [play] = useSound(boom, { interrupt: true });
  const [playChoose] = useSound(ch, { interrupt: true });
  const [playOver] = useSound(over, { interrupt: true });
  const [playWon] = useSound(won, { interrupt: true });

  const [playWinner] = useSound(winner);
  const [lostPlay] = useSound(lost);
  ...
  const [opponentMovingLoad, setOpponentMovingLoad] = useState(false);
  ...
  const getCompoundSuggestionFromOpenAI = async () => {
        ...
        messages: [
          {
            ...
            content: [
              {
                type: "text",
                text: "You are very good at chemistry and chemical compounds and elements",
              },
            ],
          },
          {
            ...
            content: [
              {
                type: "text",
                text: `${droppedItems.map(
                  (item, index) =>
                    "Element " + parseInt(index + 1) + ": " + item?.name + "\n"
                )}\nGenerate combinations of compound that is chemically possible with the given elements`,
              },
            ],
          },
        ],
            ...
            schema: {
              ...
              properties: {
                combinations: {
                  type: "array",
                  description:
                    "A list of combinations, each consisting of a symbol and a name.",
                  items: {
                    type: "object",
                    properties: {
                      symbol: {
                        type: "string",
                        description: "The symbol for the combination.",
                      },
                      name: {
                        type: "string",
                        description:
                          "The name associated with the combination.",
                      },
                      elements: {
                        type: "array",
                        description:
                          "All the elements used to make the combination.",
                        items: {
                          type: "object",
                          description:
                            "Number of times the element is repeated in the combination.",
                          properties: {
                            element: {
                              type: "string",
                              description:
                                "The element used in the combination.",
                            },
                            occurences: {
                              type: "integer",
                              description:
                                "The number of times that the element has been used in the compound.",
                            },
                          },
                          required: ["element", "occurences"],
                          additionalProperties: false,
                        },
                      },
                    },
                    required: ["symbol", "name", "elements"],
                    additionalProperties: false,
                  },
                },
              },
    ...
    .then(data => { 
        ...
        setCompoundSuggestions(JSON.parse(data.choices[0].message.content)?.combinations);
        setDroppedItems([]);
     })
  ...
  const _handleDrop = e => {
    e.stopPropagation();
    playChoose();
    setDroppedItems(prevItems => [...prevItems, e.dragData]);
  ...
  const handleDrop = e => {
    e.stopPropagation()
    setTimeout(() => {
        generateReactionFromOpenAI(null, e.dragData);
        // getOpponentMoveFromOpenAI(e.dragData);
    }, 2000)
    setPlaygroundItems(prevItems => [...prevItems, e.dragData]);
    // e.containerElem.style.visibility = "hidden";
  };
  ...
  const getOpponentMoveFromOpenAI = async (item) => {
    setOpponentMovingLoad(true)
    fetch('https://api.openai.com/v1/chat/completions',{
      method: "POST",
      headers: {
        "Content-Type": "application/json",
  ...
  "messages": [
    ...
    },
    {
      ...
      "content": [
        {
          "type": "text",
          "text": `Compound: ${item?.name} (${item?.symbol})\nGenerate a suitable simple compound that can form another compound with the given compound either by destroying it or taking parts of it.`
        }
      ]
    }
  ],
        ...
        "properties": {
          "move": {
            ...
            "properties": {
              "compound": {
                "type": "string",
                "description": "The name of the compound involved in the movement."
              },
              "elements": {
                "type": "array",
                "description": "A list of elements involved in the compound.",
                "items": {
                  "type": "object",
                  "properties": {
                    "element": {
                    ...
                    "occurences": {
                      "type": "integer",
                      "description": "The number of occurrences of this element in the compound."
                    }
                  },
                  "required": [
                    "element",
  ...
  "presence_penalty": 0
    ...
    .then(data => { 
        console.log(JSON.parse(data.choices[0].message.content));
        setOpponentItem(JSON.parse(data.choices[0].message.content)?.move);
        setTimeout(() => {
            generateReactionFromOpenAI(JSON.parse(data.choices[0].message.content)?.move, item)
        }, 3000)
     })
    .catch(error => { console.error(error); })
    .finally(() => {
        setOpponentMovingLoad(false)
    })
  }

  const generateReactionFromOpenAI = async (item=null, item2) => {
...
"messages": [
{
    ...
    {
        "type": "text",
        "text": `You are the AI opponent in a turn-based chemistry battle game. The user gives a compound or molecule. You must respond with:

1. A counter-compound that reacts (or attempts to react) with the user's compound.
2. The resulting compound or mixture formed from that reaction.
3. The result of the battle (who wins: user or AI).
4. A one-line explanation of why the winner won.

Your output **must strictly follow** this JSON format:

{
"reaction": {
"compound": "<AI's counter-compound with chemical formula and name>",
"resultant_compound": "<compound or mixture formed from the reaction>",
"result": {
    "winner": 1 or 2,  // 1 = user won, 2 = AI won
    "reason": "<concise explanation of the outcome>"
}
}
}

⚠️ Rules:
- About 30% of the time, give a weak or inert counter-compound so the user wins.
- In the rest, try to pick a scientifically plausible counter-compound that neutralizes or reacts strongly with the user’s compound.
- Use common chemical reactions like acid-base, redox, or precipitation.
- If there's no reaction, set 'resultant_compound' to "No reaction" and adjust 'winner'/'reason' accordingly.
- DO NOT return anything except the strict JSON object as described.
...
"resultant_compound": "Sodium Chloride (NaCl) and Water (H₂O)",
"result": {
    "winner": 2,
    "reason": "Strong base neutralized the acid, forming salt and water."
}
}
}

Only output valid JSON in the specified schema.`
    }
    ]
},
{
    ...
    {
        "type": "text",
        // "text": `Compound 1: ${item2?.name} (${item2?.symbol})\nCompound 2: ${opponentItem?.compound||item?.compound}\nGenerate a possible output compound with the reaction of both Compounds.  Also based on how many elements each input molecule loses in the reaction, find the winner, where winner is the element that loses fewer elements. Explain the reaction as formula as well.`
        "text": `Compound: ${item2?.name} (${item2?.symbol})`
    }
    ]
}
],
...
"json_schema": {
    ...
    "properties": {
        ...
        "properties": {
            "compound": {
            "type": "string",
            "description": "The name of the resulting compound involved in the reaction with symbol."
            },
            "resultant_compound": {
            "type": "string",
            "description": "The resulting compound or mixture formed from the reaction."
            },
            "result": {
            "type": "object",
            "properties": {
                "winner": {
                "type": "integer",
                "description": "Compound 1 or 2"
                },
                "reason": {
                "type": "string",
                "description": "The reason why that compound is a winner."
                }
            },
            "required": [
                "winner",
...
"presence_penalty": 0
    ...
    .then(async (data) => {
        setReaction(JSON.parse(data.choices[0].message.content)?.reaction);
        const engineResult = JSON.parse(data.choices[0].message.content)?.reaction;
        await addHistory(`${item2?.name} (${item2?.symbol})`, engineResult?.compound, compoundSuggestions?.length===0?droppedItems?.map(i => i.name)?.join(', '):[...new Set(compoundSuggestions.flatMap(item => item.elements.map(e => e.element)))].join(', '), engineResult?.result?.reason, engineResult?.resultant_compound, engineResult?.result?.winner===1?'You':'AI');
        if (JSON.parse(data.choices[0].message.content)?.reaction?.result?.winner !== 1) {
            playOver()
            setUserHP(prevHP => prevHP - 20)
        } else {
            playWon()
            setAiHP(prevHP => prevHP - 20)
        }
        setPlaygroundItems([])
  ...
  const resetCurrentPlay = (e) => {
    e.stopPropagation(); 
    if (userHP === 0) {
        // user lost
        lostPlay();
        document.getElementById('dialog-rounded-lost').showModal()
    } else if (aiHP === 0) {
        // ai lost
        playWinner();
        document.getElementById('dialog-rounded-won').showModal()
    } else {
  ...
  const addHistory = async (userCompound, aiCompound, molecules, reason, resultantCompound, winner) => {
    const { error } = await supabase.from('history').insert({
        user_compound: userCompound,
        molecules_involved: molecules,
        ai_compound: aiCompound,
        resultant_compound: resultantCompound,
        winner: winner,
        reason: reason
    });
    if (error) {
  ...
  return (
    ...
    >
      <div className="w-full h-screen bg-orange-200 flex flex-row items-center justify-center space-x-2 p-4 overflow-hidden">
        <dialog class="nes-dialog is-rounded m-auto" id="dialog-rounded-won">
            <form method="dialog">
                <i class="nes-icon trophy is-medium"></i>
                <p class="title">You Won!</p>
                <p>Let's summarize what you learnt</p>
                <menu class="dialog-menu">
                    <button onClick={(() => router('/summary'))} class="nes-btn is-primary">Continue</button>
                </menu>
            </form>
        </dialog>
        <dialog class="nes-dialog is-rounded m-auto" id="dialog-rounded-lost">
            <form method="dialog">
                <i class="nes-icon is-medium is-half heart"></i>
                <p class="title">You Lost!</p>
                <p>Let's summarize what you learnt</p>
                <menu class="dialog-menu">
                    <button onClick={(() => router('/summary'))} class="nes-btn is-error">Continue</button>
                </menu>
            </form>
        </dialog>
        <div className="w-[10%] h-full border-4 bg-orange-300 border-black rounded-md p-1 flex flex-col">
          <div className="w-full flex items-center justify-center rounded-md bg-white py-1">
            <p className="section-title w-full text-center text-black text-sm tracking-tighter font-medium">
              Elements
            </p>
          </div>
          <div className="w-full h-full overflow-y-auto py-2 flex flex-col items-center space-y-2 relative">
            {elements.map((element, index) => (
              <DragDropContainer
                ...
                dragData={element}
              >
                <div className={`w-[80px] nes-btn rounded-md flex items-center text-black bg-[${element.color}] justify-center`}>
                  {element?.symbol}
                </div>
              </DragDropContainer>
            ))}
        ...
        <div className="w-[30%] h-full border-4 bg-orange-300 border-black rounded-md p-1 relative flex flex-col items-center space-y-2 justify-between pb-2">
          <div className="w-full flex items-center justify-center rounded-md bg-white py-1">
            <p className="section-title w-full text-center text-black text-sm tracking-tighter font-medium flex items-center justify-center space-x-2">
              <svg
                ...
                strokeWidth={2}
                stroke="currentColor"
                className="fill-red-500 text-black size-5"
              >
                <path
                  strokeLinecap="round"
          ...
          <div className="w-full h-full">
            ...
            <DropTarget targetKey="foo" onHit={_handleDrop}>
              <div className="w-full h-full py-2 flex flex-col items-center space-y-2 p-2">
                {droppedItems.map((item, index) => (
                  <DragDropContainer targetKey="bar" dragData={item}>
                  <div
                    key={index*index+'dropped'+Math.random()}
                    className={`w-full nes-btn px-2 bg-[${item?.color}] rounded-md mb-2`}
                  >
              ...
              <div className="w-full h-full flex flex-col items-center space-y-2">
              {
                  compoundSuggestions.map((compound, index) => (
                      <DragDropContainer targetKey="bar" dragData={compound}>
                      <div
                        key={index+index+'duggestions'+Math.random()}
                        className="w-full flex flex-row items-center nes-btn justify-between px-2 py-1 bg-white rounded-md border-4 border-black"
                      >
          ...
          </div>
          {
              droppedItems.length > 0 && (
                  <button
            onClick={() => getCompoundSuggestionFromOpenAI()}
            type="button"
        ...
        <div className={`w-[60%] ${reactionLoading?'transform -scale-50 animate-spin duration-400':''} h-full border-4 bg-orange-300 border-black rounded-lg p-1 relative`}>
          <div className="w-full flex items-center justify-center rounded-md bg-white py-1">
            <p className="section-title w-full text-center text-black text-sm tracking-tighter font-medium flex items-center justify-center space-x-2">
              <svg
                xmlns="http://www.w3.org/2000/svg"
          ...
          <div className="h-full">
              <DropTarget key={resetKey} targetKey="bar" onHit={handleDrop}>
                  <div className="w-full h-full py-2 relative">
                      {
                          reaction?.compound && (
                              <span className="absolute top-[20%] nes-btn right-4 text-black font-medium px-2 py-2 bg-white rounded-md border-4 border-black">
                                  {reaction?.compound}
                                  <span className="absolute -top-7 right-0 bg-red-100 border-4 border-black rounded-md text-md text-black font-medium">Ai</span>
                              </span>
                          )
                      }
                      {
                          reaction?.resultant_compound && (
                              <span className="absolute nes-btn is-warning top-[30%] text-black font-medium px-2 py-2 bg-white rounded-md border-4 border-black">
                                  {reaction?.resultant_compound}
                              </span>
                          )
                      }
                      {
                          reaction?.result && (
                              <div
                                className={`absolute top-[35%] nes-btn ${reaction?.result?.winner===1?'is-success':'is-error'} text-black font-medium px-2 py-2 rounded-md border-4 border-red-500 bg-red-100`}
                                style={{ zIndex: 50, pointerEvents: 'auto' }}
                              >
                                {reaction?.result?.winner===1?'You':'Ai'||'N/A'} made a strike! 
                                <button type="button" onClick={resetCurrentPlay} style={{ zIndex: 100, pointerEvents: 'auto' }} className="nes-btn">Continue</button>
                              </div>
                          )
                      }
                      {
                          reaction?.result && (
                              <span className="absolute top-[40%] nes-btn text-black font-medium px-2 py-2 bg-white rounded-md border-4 border-black">
                                  Why: {reaction?.result?.reason||''}
                              </span>
                          )
                      }
                  </div>
          ...
          <div className="w-full -ml-[4px] absolute -bottom-1 flex flex-col items-center justify-between">
            <div className="w-full"></div>
            <div className="w-full flex flex-row items-center justify-between px-3 space-x-2">
              <div className="w-1/2 flex flex-row items-center">
                <div className="flex items-center justify-center nes-btn bg-orange-100 rounded-md">
                  You
                </div>
                <progress
